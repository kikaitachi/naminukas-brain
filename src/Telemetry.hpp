#ifndef SRC_TELEMETRY_HPP_
#define SRC_TELEMETRY_HPP_
...
class ItemCommand: public Item {
 public:
   ...
   */
  ItemCommand(int parent_id, std::string name, std::string value,
...
class Item3DModelBase : public Item {
 public:
  Item3DModelBase(int parent_id, int type, std::string name,
    uint32_t color, std::vector<Transform> transforms);
  void serialize_definition(void **buf, int *buf_len);
  void serialize_value(void **buf, int *buf_len);
...
class Item3DModel: public Item3DModelBase {
 public:
  Item3DModel(int parent_id, std::string name, std::string mime_type,
...
class Item3DModelRef: public Item3DModelBase {
 public:
  Item3DModelRef(int parent_id, std::string name, int ref_id,
    uint32_t color, std::vector<Transform> transforms);
  void serialize_definition(void **buf, int *buf_len);
 private:
  int ref_id;
...
class ItemChoice: public Item {
 public:
  ItemChoice(int parent_id, std::string name,
